...
void LogCore(int Level, const char* msg, const unsigned char * data=NULL, size_t datalen=0){
    ...
    if(data!=NULL && datalen>0){
        // Define a sufficiently large buffer for msg + hex data (adjust size if needed)
        char newmsg[512]; // 512 is arbitrary, increase if needed

        // Copy original message first
        int offset = snprintf(newmsg, sizeof(newmsg), "%s ", msg);

        // Append hex bytes of data
        for (size_t i = 0; i < datalen && offset + 3 < sizeof(newmsg); i++)
        {
            offset += snprintf(newmsg + offset, sizeof(newmsg) - offset, "%02x ", data[i]);
        }

        //ESP_LOGI("HCIE", "%s", newmsg);
    }else{
        //Serial.println(msg);
        ESP_LOGI("HCIE", "%s", msg);
    }
...
#define readCRC(arr, length) \
    ((uint16_t)(arr[(length - MODBUS_CRC_LENGTH) + 1]) << 8 | (uint16_t)(arr[length - MODBUS_CRC_LENGTH]))

#define readUInt16(arr, index) \
    ((uint16_t)(arr[(index)]) << 8 | (uint16_t)(arr[(index) + 1]))
/**
...
void HCIEmulator::poll(){
    ...
    if (available > 0)
    {
        m_rxlen += uart_read_bytes(UART_NUM_1, (char *)(m_rxbuffer + m_rxlen), min((int)(255 - m_rxlen), available), 0);
        if(m_rxlen > 254)
        {
    ...
    if(m_rxlen>0 && (micros()-m_recvTime > T3_5)) 
    {
        ...
        if(!m_skipFrame){
            ...
            if(m_txlen > 0){
                ...
                // Set DE and RE high to enable transmitter mode
                ...
                uart_write_bytes(UART_NUM_1, m_txbuffer, m_txlen);
...
void HCIEmulator::processBroadcastStatusFrame(){
    ...
    bool doorStateHasChanged = false;
...
void HCIEmulator::closeDoor(){
    ...
    if (m_state.doorState == DOOR_MOVE_OPENPOSITION)
    {
        m_closeTwice = true;
    }
    else
